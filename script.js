...
/* ========================================================
   ...
   Config is loaded dynamically from localStorage so you
   never have to hardcode credentials in this file.
...
function saveFirebaseConfig(cfg) {
    try {
        window.localStorage.setItem('traderZenFirebaseConfig', JSON.stringify(cfg));
        return true;
    } catch (e) {
        return false;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const fbAuthDomain = document.getElementById('fb-auth-domain');
    const fbProjectId = document.getElementById('fb-project-id');
    const fbStorageBucket = document.getElementById('fb-storage-bucket');
    const fbMessagingSenderId = document.getElementById('fb-messaging-sender-id');
    ...
    const fbConfiguredBadge = document.getElementById('fb-configured-badge');
    ...
    function switchView(viewName) {
        ...
        if (viewName === 'dashboard') {
            ...
            // Load saved Firebase config into fields
            const savedCfg = loadFirebaseConfig();
            if (savedCfg) {
                if (fbApiKey) fbApiKey.value = savedCfg.apiKey || '';
                if (fbAuthDomain) fbAuthDomain.value = savedCfg.authDomain || '';
                if (fbProjectId) fbProjectId.value = savedCfg.projectId || '';
                if (fbStorageBucket) fbStorageBucket.value = savedCfg.storageBucket || '';
                if (fbMessagingSenderId) fbMessagingSenderId.value = savedCfg.messagingSenderId || '';
                if (fbAppId) fbAppId.value = savedCfg.appId || '';
            }
            // Show configured badge if Firebase is active
    ...
    if (saveFirebaseBtn) {
        saveFirebaseBtn.addEventListener('click', () => {
            ...
            if (!cfg.apiKey || !cfg.projectId || !cfg.appId) {
                if (firebaseStatus) {
                    firebaseStatus.textContent = "Please fill in at least the API Key, Project ID, and App ID.";
            ...
            const saved = saveFirebaseConfig(cfg);
            if (saved) {
                if (firebaseStatus) {
                    firebaseStatus.innerHTML = '<strong>Config saved!</strong> Reloading in 2 seconds...';
                    firebaseStatus.style.color = "#10b981";
                    firebaseStatus.style.display = "block";
                }
                if (fbConfiguredBadge) fbConfiguredBadge.style.display = 'inline-flex';
                    setTimeout(() => { window.location.reload(); }, 2000);
            } else {
                if (firebaseStatus) {
                    firebaseStatus.textContent = "Failed to save config. Please check your browser settings.";
                    firebaseStatus.style.color = "#ef4444";
                    firebaseStatus.style.display = "block";
                }
            }
